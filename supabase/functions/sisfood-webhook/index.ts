...
serve(async (req) => {
    ...
    try {
        ...
        const isServiceRole = providedToken === Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');

        // Qualquer token não-vazio é aceito (Tampermonkey envia a ANON_KEY).
        // Apenas bloqueamos chamadas SEM token algum (aviso) ou com token claramente inválido (vazio).
        if (!providedToken) {
           console.warn(`⚠️ [SECURITY] Chamada sem token pela loja ${loja}.`);
        } else {
           console.log(`🔑 [AUTH] Token recebido para loja ${loja}. Admin: ${isServiceRole}`);
        }
