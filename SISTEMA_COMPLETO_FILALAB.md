...
- **Últimas Implementações**:
    - **Transmissão Automatizada e Telemetria de Desempenho (FPS Real & Resolução) com Trava de 60 FPS**: 
        - **Sincronização Automática na Franquia**: Agora, ao iniciar uma transmissão de tela de qualquer unidade (ex: Itaquá), a plataforma busca todas as unidades da mesma franquia no Supabase e insere automaticamente a mídia de `tipo: 'transmissao'` na playlist de screensaver (`tv_playlist`) de cada uma delas (Itaquá, Poá, Suzano, etc.). Quando a transmissão é finalizada (ou se a aba do transmissor for recarregada/fechada de surpresa), a mídia é removida automaticamente de todas as playlists, garantindo que as TVs voltem à rotatividade normal sem intervenção manual.
        ...
        - **Telemetria de FPS e Resolução**: Adicionado um HUD informativo e flutuante de diagnóstico no Preview de transmissão local (`ScreenShareTransmitter.tsx`) que calcula e exibe em tempo real o **FPS Real** renderizado pelo navegador (via `requestVideoFrameCallback` com fallback para `requestAnimationFrame`) e a resolução/FPS nominal da captura. A taxa máxima está configurada em **60 FPS** (`frameRate: { ideal: 60, max: 60 }`), permitindo oscilação automática conforme o desempenho.
