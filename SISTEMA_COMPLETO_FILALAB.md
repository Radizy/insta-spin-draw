...
### Versão do Sistema
- **Versão Atual**: `2.9.0` (Junho 2026)
- **Últimas Implementações**:
    - **Transmissão com Telemetria de Desempenho (FPS Real & Resolução) e Trava Adaptativa de 30-60 FPS**: Adicionado um HUD informativo e flutuante de diagnóstico na tela de Preview de transmissão local (`ScreenShareTransmitter.tsx`). O painel calcula e exibe em tempo real o **FPS Real** renderizado pelo navegador (utilizando a API de alto desempenho `requestVideoFrameCallback` com fallback dinâmico para `requestAnimationFrame`) juntamente com a resolução nativa e taxa de frames **Nominal** fornecidas pelas configurações do track de vídeo (`getSettings()`). Para garantir a fluidez ideal e permitir a oscilação de taxa caso o processamento do computador transmissor fique sobrecarregado, configuramos uma faixa adaptativa de **30 a 60 FPS** nas restrições da API `getDisplayMedia` (`frameRate: { ideal: 60, min: 30, max: 60 }`). Essa ferramenta de diagnóstico ajuda o operador a identificar rapidamente se engasgos ou travamentos na TV receptora são causados por limitações do computador transmissor (baixa taxa de captura local) ou por sobrecarga da CPU/Hardware decodificador da própria Smart TV (que costuma rodar a 20-25 FPS por gargalo térmico/rede).
