...
- **Últimas Implementações**:
    - **Transmissão com Telemetria de Desempenho (FPS Real & Resolução) e Trava de 60 FPS**: Adicionado um HUD informativo e flutuante de diagnóstico na tela de Preview de transmissão local (`ScreenShareTransmitter.tsx`). O painel calcula e exibe em tempo real o **FPS Real** renderizado pelo navegador (utilizando a API de alto desempenho `requestVideoFrameCallback` com fallback dinâmico para `requestAnimationFrame`) juntamente com a resolução nativa e taxa de frames **Nominal** fornecidas pelas configurações do track de vídeo (`getSettings()`). Para garantir a fluidez ideal, limitamos a taxa de frames máxima de captura de tela em **60 FPS** (`frameRate: { ideal: 60, max: 60 }`), permitindo que a taxa oscile naturalmente para baixo dependendo da carga de processamento e de mudanças na tela sem quebras na API de mídia. Essa ferramenta de diagnóstico ajuda o operador a identificar rapidamente se engasgos ou travamentos na TV receptora são causados por limitações do computador transmissor (baixa taxa de captura local) ou por sobrecarga da CPU/Hardware decodificador da própria Smart TV (que costuma rodar a 20-25 FPS por gargalo térmico/rede).
