...
- **Últimas Implementações**:
    ...
    - **Integração SISFOOD v11.0 (Anti-Zumbi)**: Script Tampermonkey `WebhookConfig.tsx` reescrito para proteger contra reciclagens indevidas de comanda dupla no PDV (`_filaAtualSisfood`). A arquitetura da Matriz de Itaquá foi separada em uma Rota Batch otimizada nativa de lista, enquanto as franquias atuam com API dupla sequencial rápida. O *polling de dispatcher* da fila baixou de 10s para ágeis `4000ms`, garantindo tempo de alerta na TV imediato.
