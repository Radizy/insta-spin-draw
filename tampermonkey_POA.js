// ==UserScript==
// @name         Integração SISFOOD x FilaLab (LOJA POA)
// @namespace    http://tampermonkey.net/
// @version      7.0 - POA
// @description  Lê a fila de "Poá" e manda pro FilaLab
...
(function() {
            ...
            if (this._url && this._url.includes('/listarJson')) {
                try {
                    ...
                    if (hashAtual !== ultimaHashFila || contagemFila !== ultimaContagemFila) {
                        console.log(`🟢 [FILALAB POA] Fila: ${contagemFila} pedidos. Disparando Supabase!`);
    ...
    async function enviarFilaLab(lojaNome, filaCount, pedidosFila) {
        try {
            ...
            if (resposta.ok) {
                 console.log(`✅ [FILALAB POA] Sucesso! Banco atualizado (${filaCount}).`);
            }
        } catch (e) {
