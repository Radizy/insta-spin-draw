// ==UserScript==
// @name         Integração SISFOOD x FilaLab (LOJA SUZANO)
// @namespace    http://tampermonkey.net/
// @version      7.0 - SUZANO
// @description  Lê a fila de "Suzano" e manda pro FilaLab
...
(function() {
            ...
            if (this._url && this._url.includes('/listarJson')) {
                try {
                    ...
                    if (hashAtual !== ultimaHashFila || contagemFila !== ultimaContagemFila) {
                        console.log(`🟢 [FILALAB SUZANO] Fila: ${contagemFila} pedidos. Disparando Supabase!`);
    ...
    async function enviarFilaLab(lojaNome, filaCount, pedidosFila) {
        try {
            ...
            if (resposta.ok) {
                 console.log(`✅ [FILALAB SUZANO] Sucesso! Banco atualizado (${filaCount}).`);
            }
        } catch (e) {
